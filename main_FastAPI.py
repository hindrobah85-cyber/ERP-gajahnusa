...
from backend.app.models.database import (
    ...
    User, Customer, Order, Nota, Payment, 
    SalesVisit, Delivery, Product, OrderItem,
    FraudDetectionLog,
    UserRole, PaymentStatus, NotaStatus, OrderStatus
)
...
async def get_customers(
    ...
    if current_user.role == UserRole.SALES_TOKO:
    ...
    elif current_user.role == UserRole.SALES_PROJECT:
...
async def sales_checkout(
    ...
    visit.duration_minutes = int((visit.check_out - visit.check_in).total_seconds() / 60)
    visit.notes = notes
    visit.competitor_info = competitor_info
...
async def create_nota(
    ...
    # Generate unique QR
    qr_hash, qr_image = payment_service.generate_nota_qr(order_id, nota_number, order.total_amount)
...
async def initiate_payment(
    ...
    # Check salesman fraud score
    if current_user.fraud_score > Config.FRAUD_THRESHOLD:
        raise HTTPException(
            status_code=403, 
            detail="Account flagged for review. Please contact supervisor."
        )
    
    result = await payment_service.process_payment(payment_data, current_user.id, db)
...
async def confirm_deposit(
    ...
    payment.deposited_at = datetime.utcnow()
    payment.status = PaymentStatus.VERIFIED
    
    # Calculate delay
    hours_delay = (payment.deposited_at - payment.created_at).total_seconds() / 3600
    payment.late_deposit_hours = max(0, hours_delay - 24)
    
    # Update nota status
    nota = db.query(Nota).filter(Nota.id == payment.nota_id).first()
    if nota:
        nota.status = NotaStatus.PAID
        nota.payment_date = datetime.utcnow()
    
    # Update salesman fraud score if late
    if payment.late_deposit_hours > 0:
        salesman = db.query(User).filter(User.id == payment.salesman_id).first()
        if salesman:
            salesman.fraud_score = min(salesman.fraud_score + 0.05, 1.0)
...
async def create_order(
    ...
    try:
        ...
        # Check credit limit
        if customer.credit_used + total_amount > customer.credit_limit:
            raise HTTPException(
                status_code=400, 
                detail=f"Order exceeds credit limit. Available: {customer.credit_limit - customer.credit_used}"
            )
...
async def sales_dashboard(
    ...
    elif current_user.role == UserRole.SUPERVISOR_TOKO:
    ...
    return {
        ...
        "collections": {
            "total_payments": payment_result.total_payments or 0,
            "total_amount": float(payment_result.total_collected or 0)
        }
    }
