...
class ApiService {
  ...
  private _offlineQueue: Array<{ method: string; endpoint: string; data: any; timestamp: number }>;
  ...
  get offlineQueue(): Array<{ method: string; endpoint: string; data: any; timestamp: number }> {
  ...
  async getToken(): Promise<string | null> {
    if (!this._token) {
      this._token = await SecureStore.getItemAsync('authToken');
    }
    return this._token;
  }

  async request(method: string, endpoint: string, data: any = null, requiresAuth: boolean = true): Promise<any> {
    const config: any = {
      method,
    ...
    if (!this._isOnline && method !== 'GET') {
      this._offlineQueue.push({ method, endpoint, data, timestamp: Date.now() });
      await SecureStore.setItemAsync('offlineQueue', JSON.stringify(this._offlineQueue));
  ...
  async syncOfflineData(): Promise<void> {
    const queuedData = await SecureStore.getItemAsync('offlineQueue');
    if (queuedData) {
      this._offlineQueue = JSON.parse(queuedData);
      
      for (const request of this._offlineQueue) {
        try {
      ...
      this._offlineQueue = [];
      await SecureStore.deleteItemAsync('offlineQueue');
    }
  }
}
...
// ============= LOGIN SCREEN =============
const LoginScreen = ({ onLogin, onRegister }: { onLogin: any; onRegister: any }) => {
    ...
    <LinearGradient colors={['#1a1a2e', '#16213e']} style={styles.container}>
      <ScrollView contentContainerStyle={styles.scrollContainer}>
        <View style={styles.logoContainer}>
          <GajahNusaLogo size={100} />
          <Text style={styles.appTitle}>GAJAH NUSA</Text>
          <Text style={styles.appSubtitle}>ERP Anti-Fraud System</Text>
        </View>
        ...
        <View style={styles.formContainer}>
          ...
          >
            <MaterialCommunityIcons name="face-recognition" size={24} color="#0f9d58" />
    ...
    <View style={styles.container}>
      ...
      >
        ...
        <View style={styles.statsContainer}>
          ...
          <View style={styles.statCard}>
            <MaterialCommunityIcons name="cart-outline" size={30} color="#2196F3" />
            <Text style={styles.statValue}>{stats.orders}</Text>
            <Text style={styles.statLabel}>Orders</Text>
          </View>
          
          <View style={styles.statCard}>
            <MaterialCommunityIcons name="cash-multiple" size={30} color="#FF9800" />
            <Text style={styles.statValue}>{stats.collections.toLocaleString()}</Text>
            <Text style={styles.statLabel}>Collections</Text>
          </View>
          
          <View style={styles.statCard}>
            <MaterialCommunityIcons name="target" size={30} color="#9C27B0" />
      ...
      {showScanner ? (
        ...
        <ScrollView style={styles.visitContent}>
          {!checkInData ? (
            ...
            >
              <MaterialCommunityIcons name="qrcode-scan" size={60} color="#0f9d58" />
...
const PaymentScreen = ({ user, onBack }) => {
  ...
  const handleTakePhoto = async () => {
    ...
    if (!result.canceled) {
      setReceiptPhoto(result.assets[0].base64);
      ...
      <ScrollView style={styles.paymentContent}>
        {!showOtpInput ? (
          ...
          <View style={styles.otpContainer}>
            <MaterialCommunityIcons name="shield-lock" size={60} color="#0f9d58" />
        ...
        <View style={styles.section}>
          ...
          {products.map(product => (
            ...
            >
              ...
              <MaterialCommunityIcons name="plus-circle" size={28} color="#0f9d58" />
            </TouchableOpacity>
          ))}
        </View>
